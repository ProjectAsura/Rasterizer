...
{
    const char16* filename = L"depth.bmp";
    ...
    for( size_t Index=0; Index < vertices.size(); Index += 3 )
    {
        ...
        // ラスタライズ処理.
        {
            auto vs1 = ToVector2(P1p) - ToVector2(P0p);
            auto vs2 = ToVector2(P2p) - ToVector2(P0p);
